...
def BOB_news_phase_finite_t0_numerically(Omega_0,Omega_QNM,t_array,t0_tp_tau,t_tp_tau,Phi_0=0):
    Omega = BOB_news_freq_finite_t0(Omega_0,Omega_QNM,t0_tp_tau,t_tp_tau)

    if(Omega[0] == -1):
        print("BAD OMEGA_0")
        return -1
    
    Phase = cumulative_trapezoid(Omega,t_array,initial=0)
...
def BOB_psi4_freq_finite_t0(Omega_0,Omega_QNM,t0_tp_tau,t_tp_tau):
    ...
    X = Omega_0**4 + k*(np.tanh(t_tp_tau) - np.tanh(t0_tp_tau))
    if(np.min(X)<0):
        print("Imaginary Frequency Obtained Due To Bad Omega_0")
        return np.full_like(X,-1)
    Omega = (X)**0.25
    return Omega
def BOB_psi4_phase_finite_t0(Omega_0,Omega_QNM,tau,t0_tp_tau,t_tp_tau,Phi_0=0):
...
def construct_BOB_finite_t0(chif,mf,l,m,t0,Omega_0Ap,tp,what_to_create,start_before_tpeak = -50, end_after_tpeak = 50):
    ...
    if(what_to_create=="strain" or what_to_create=="h"):
        Phi = BOB_strain_phase_finite_t0(Omega_0,Omega_QNM,tau,t0_tp_tau,t_tp_tau)
        phase = m*Phi
    elif(what_to_create=="news" or what_to_create=="n"):
        Phi = BOB_news_phase_finite_t0_numerically(Omega_0,Omega_QNM,t,t0_tp_tau,t_tp_tau)
        phase = m*Phi
    elif(what_to_create=="psi4"):
        Phi = BOB_psi4_phase_finite_t0(Omega_0,Omega_QNM,tau,t0_tp_tau,t_tp_tau)
        phase = m*Phi
    else:
