...
def BOB_strain_freq_finite_t0_sym(t, t0, tp, tau, Omega0, Omega_QNM):
    x = (t - tp) / tau
    x0 = (t0 - tp) / tau
    return Omega_QNM * (Omega0 / Omega_QNM)**((sp.tanh(x) - 1) / (sp.tanh(x0) - 1))
...
def BOB_strain_phase_finite_t0_sym(t, t0, tp, tau, Omega0, Omega_QNM, Phi_0):
    ...
    term1 = Omega_ratio**(sp.Integer(2) / (sp.tanh(-x0) + 1))
    log_term = -sp.log(Omega_ratio) / (sp.tanh(x0) - 1)
    term2 = log_term * (sp.tanh(x) + 1); term3 = log_term * (sp.tanh(x) - 1)
    return outer * (term1 * Ei_sp(term2) - Ei_sp(term3)) + Phi_0, Omega

def BOB_news_freq_finite_t0_sym(t, t0, tp, tau, Omega0, Omega_QNM):
    x = (t - tp) / tau
    x0 = (t0 - tp) / tau
    F = (Omega_QNM**2 - Omega0**2) / (1 - sp.tanh(x0))
    return sp.sqrt(Omega_QNM**2 + F * (sp.tanh(x) - 1))
...
def BOB_psi4_freq_finite_t0_sym(t, t0, tp, tau, Omega0, Omega_QNM):
    ...
    return (Omega0**4 + k * (sp.tanh(x) - sp.tanh(x0)))**sp.Rational(1, 4)
...
def BOB_psi4_phase_sym(t, tp, Omega0, Omega_QNM, tau, Phi_0):
    ...
    Omega_minus_q0 = Omega_QNM - Omega0; Omega_plus_q0 = Omega_QNM + Omega0
