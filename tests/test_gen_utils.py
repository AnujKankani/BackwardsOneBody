...
#     print(strain.real().t)
#     print(strain_psi4.real().t)
#     # Load reference
#     ref = np.load("tests/trusted_outputs/kuibit_cce9_rhOverM_R0472_l2_m0.npz")

#     # Compare arrays
#     np.testing.assert_allclose(strain.real().t, ref["x_real"], rtol=1e-12, atol=1e-15)
#     np.testing.assert_allclose(strain.real().y, ref["y_real"], rtol=1e-12, atol=1e-15)
#     np.testing.assert_allclose(strain.imag().y, ref["y_im"], rtol=1e-12, atol=1e-15)
#     np.testing.assert_allclose(strain_psi4.real().t, ref["x_real"], rtol=1e-12, atol=1e-15)
#     np.testing.assert_allclose(strain_psi4.real().y, ref["y_real"], rtol=1e-12, atol=1e-15)
#     np.testing.assert_allclose(strain_psi4.imag().y, ref["y_im"], rtol=1e-12, atol=1e-15)
def test_kuibit(strain_h5, l = 2, m = 0):
    # Add these print statements for debugging
    import sxs
    print(f"\n--- DEBUGGING test_kuibit ---")
    print(f"SXS version: {sxs.__version__}")
    print(f"Type of loaded waveform object: {type(strain_h5)}")
    print(f"Shape of waveform data array: {strain_h5.data.shape}")
    print(f"Length of time array: {len(strain_h5.t)}")
    
    index = strain_h5.index(l, m)
    print(f"Index for (l={l}, m={m}): {index}")
    
    strain = gen_utils.get_kuibit_lm(strain_h5,l,m)
    print(f"Shape of resulting kuibit timeseries: {strain.y.shape}")
    
    # Load reference
    ref = np.load("tests/trusted_outputs/kuibit_cce9_rhOverM_R0472_l2_m0.npz")
    print(f"Shape of trusted reference data: {ref['x_real'].shape}")
...
def test_get_phase(psi4_ts):
    ...
    np.testing.assert_allclose(ts.real().t, ref["x_real"], rtol=1e-12, atol=1e-15)
    np.testing.assert_allclose(ts.real().y, ref["y_real"], rtol=1e-12, atol=1e-15)
    np.testing.assert_allclose(ts.imag().y, ref["y_im"], rtol=1e-12, atol=1e-15)
...
#         assert np.isclose(result, exp, rtol=1e-12)
...
#         assert np.isclose(result_w, exp_w, rtol=1e-12)
#         assert np.isclose(result_tau, exp_tau, rtol=1e-12)
...
#     assert np.isclose(result_tp, expected_tp, rtol=1e-12)
#     assert np.isclose(result_Ap, expected_Ap, rtol=1e-12)
# def test_mismatch():
